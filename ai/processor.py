...
**技术向内容必须判高（防漏判，这是硬性要求）**：
- 本人是程序员 / 独立开发者 / SaaS 开发者 / 技术博主 / 程序员出身，靠写代码、卖软件、卖代码模板、接开发外包赚钱 → 直接 5 分
- 核心交付物是软件 / App / 网站 / API / 自动化脚本 / 代码模板 → 4 分起步
- 教别人编程、卖代码模板、做 n8n / Make / Zapier 自动化服务外包 → 4 分起步
- 只有「帮某行业用无代码工具搭流程、按月收服务费」这类，才可能是 3 分
- 反过来，「程序员转行做手工皂」「无需编程的小红书带货」这类，看的是转行后做的事，不是过去的身份
...
- **1 分**：纯卖课/卖社群/卖铲子——「我靠XX月入10万」「99元学会」「限时优惠」「名额有限」→ 标题夸张正文空洞，**且通篇没有具体可复刻的真实案例**（无模式拆解、无获客路径、无交付物、无真实数据）。

- **重要区分（避免误杀真实案例）**：很多优质一人公司作者会**顺带**推广自己的课/训练营/资料包/社群，但只要他**先分享了真实跑通的模式**（具体交付物 + 获客渠道 + 成本或收入数据），这就不是纯卖铲子，而是「真实案例顺带变现」——按案例真实度给 **3-5 分**，不要因为文末出现「课程/社群/资料」字样就压到 1 分。只有「没有真实案例、通篇教人赚钱」才算 1 分。

**判定规则**：真实性评分必须是 1-5 的整数。真实性 <= 1 → 直接 irrelevant=false（**只有纯卖铲子才杀**）。2 分及以上保留；真实案例顺带卖课（auth>=3）一律保留，可在卡片风险行标注「作者也卖课」。
...
### 红线 4：必须是一人公司，不是个体户、自由职业或流量号

一人公司**不是粉丝多**，而是**有自己的产品/服务 + 具体的服务人群**。判据不在「几个人」，
在**收入跟你本人的时间是否脱钩**。三条任一不过 → 直接淘汰：

**① 交付物必须可复用**（多服务一个客户，不需要多花你一份时间）
- 淘汰：线下开店/重资产（无人麻将馆、餐饮店、实体零售、门店、房租、装修、囤货、物流、场地）→ 杠杆是资本，那是个体户
- 淘汰：按次卖时间（上门服务、训犬、家教、接单外包、计时咨询、驻场）→ 停工即停收，那是自由职业
- 淘汰：远程全职、受雇于人 → 那是在打工
- 要选：数字产品（模板/资料包/课程/软件）、内容资产（图库/音频/内容库）、可复制的标准化交付

**② 必须有自己的产品**，不能只靠流量变现
- 淘汰：只有广告分成/带货佣金/平台补贴、没有自卖产品 → 流量号，不是一人公司
- 要选：有明确的付费产品（哪怕很简单），流量只是获客手段

**③ 交付里不能是「你的实时时间」占大头**
- 关键区分：**付费社群本身不算淘汰项**——团体形态没问题，看的是群里交付什么
  - 允许：群里交付的是**已生产好的资产**（持续更新的资料库/案例库/工具包/固定 SOP）→ 边际成本低，停手也不断收
  - 淘汰：群里交付的是**你的实时时间**（每天亲自答疑、1对1陪跑、实时督学、私人咨询为主业）→ 被人工拖死，一个人做不大
- 判据：这个社群你停更两周，成员还会续费吗？会 → 资产型，可以；不会 → 时间型，淘汰

**一句话判据：他停手两周，收入是断崖还是缓降？断崖 → 淘汰。**
**服务人群必须具体**：不能是「想做副业的人」，要是「准备考研某专业的学生」「需要配图的设计师」。
...
8. 四条一票否决红线命中任一条 → relevant=false（红线 4 = 不是一人公司：线下重资产 / 按次卖时间 / 纯流量号 / 陪跑社群为主业）
